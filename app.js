...
function createRoom() {
    ...
    db.ref(`rooms/${newId}`).set({ password: pwd, createdAt: firebase.database.ServerValue.TIMESTAMP })
        .then(() => { currentRoomId = newId; document.getElementById('roomIdDisplay').innerText = newId; showView('nicknameView'); });
}
...
function listenToRoom() {
    ...
    roomRef.child('grid').on('value', snap => {
        ...
        document.querySelectorAll('.p-btn').forEach(btn => { 
            ...
            const tag = btn.querySelector('.user-tag'); if (tag) tag.remove(); 
        });
...
function pickCustomColor() {
    ...
    db.ref(`rooms/${currentRoomId}/users`).once('value', snap => {
        const users = snap.val() || {};
        if (Object.keys(users).some(u => u !== myNickname && users[u].color === selected)) return alert("此顏色已被隊友選用！");
        myColor = selected; isLocked = true;
...
function resetMyColor() {
    if (!confirm("重選將清空你填過的所有平台，確定嗎？")) return;
    isLocked = false;
    ...
    db.ref(`rooms/${currentRoomId}/grid`).once('value', snap => {
        ...
        Object.keys(data).forEach(key => { if (data[key].user === myNickname) updates[key] = null; });
...
function togglePlatform(f, p) {
    ...
    db.ref(`rooms/${currentRoomId}/grid`).once('value', snap => {
        const data = snap.val() || {}, key = `${f}_${p}`;
        if (data[key]) { if (data[key].user === myNickname) db.ref(`rooms/${currentRoomId}/grid/${key}`).remove(); return; }
        for (let i = 1; i <= 4; i++) { if (data[`${f}_${i}`] && data[`${f}_${i}`].color === myColor) return; }
        db.ref(`rooms/${currentRoomId}/grid/${key}`).set({ user: myNickname, color: myColor });
    });
}
...
function autoFillNextFloor(p) {
    ...
    db.ref(`rooms/${currentRoomId}/grid`).once('value', snap => {
        ...
        for (let f = 1; f <= 10; f++) {
            let found = false;
            for (let i = 1; i <= 4; i++) { if (data[`${f}_${i}`] && data[`${f}_${i}`].color === myColor) { found = true; break; } }
            if (!found) { nextF = f; break; }
            if (f === 10) return;
        }
        if (data[`${nextF}_${p}`]) return;
        db.ref(`rooms/${currentRoomId}/grid/${nextF}_${p}`).set({ user: myNickname, color: myColor });
    });
}
...
function leaveRoom() {
    if (!confirm("確定要離開房間嗎？（離開後將取消同步）")) return;
...
function copyShareLink() {
    ...
    navigator.clipboard.writeText(link).then(() => alert("房號連結已複製"));
}
